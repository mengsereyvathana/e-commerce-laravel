...
const getCategory = async () => {
    ...
    if (categories.length > 0) form.value.c_id = categories[0].id;
}
...
const saveProduct = () => {
    ...
    if (form.value.image.length < 3) return Swal.fire({
        toast: true,
        position: 'top',
        showClass: {
            icon: 'animated heartBeat delay-1s'
...
<template>
    <div class='lg:py-7 lg:px-10 p-5'>
        <div class="flex justify-between items-end">
            <h1 class='text-3xl font-bold text-black_500'>Add a product</h1>
            <button @click="saveProduct()" class='px-4 py-2 rounded-md bg-primary text-white text-sm cursor-pointer'>Publish
                product</button>
        </div>
