...
const getCategory = async () => {
    const respone = await axios.get(`/api/category/${params.value.id}`);
...
function updateCategory() {
    ...
    axios.post(`/api/category/${params.value.id}`, formData, {
        headers: {
...
<template>
    <div class='lg:py-7 lg:px-10 p-5'>
        <div class="flex justify-between items-end">
            <h1 class='text-3xl font-bold text-black_500'>Edit a category</h1>
            <button class='px-4 py-2 rounded-md bg-primary text-white text-sm cursor-pointer' @click="updateCategory()">Edit
                category</button>
        </div>
