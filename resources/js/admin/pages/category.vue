...
let categoryByPage = computed(() => {
    ...
    for (let index = 0; index < categories.value.length; index++) {
        if (index >= (activePage.value - 1) * ITEM_PER_PAGE && index < activePage.value * ITEM_PER_PAGE)
            category.push(categories.value[index]);
    }
    return category;
});
...
const getCategory = async () => {
    ...
    categories.value = respone.data.result.reverse();
    page = Math.ceil(categories.value.length / ITEM_PER_PAGE);
}
...
<template>
    <div class='lg:py-7 lg:px-10 p-5'>
        <h1 class='text-3xl font-bold text-black_500'>Category</h1>
    </div>
    ...
    <div class='bg-white relative border-solid border-t border-b border-gray-200 border-t-gray-300 px-5 lg:px-10'>
        <div
            ...
            <table class='w-[800px] md:w-full'>
                ...
                <tbody>
                    <tr class='group relative border-solid border-b border-gray-300' v-for="item in categoryByPage">
                        ...
                        <td class='text-ph font-semibold text-gray-700 py-2 px-3'>
                            <p class='w-[300px] text-ph font-semibold truncate py-2 px-3'>{{ item.description
                            }}
                            </p>
                        </td>
